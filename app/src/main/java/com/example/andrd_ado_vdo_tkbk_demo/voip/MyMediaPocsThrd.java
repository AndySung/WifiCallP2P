package com.example.andrd_ado_vdo_tkbk_demo.voip;
...
import com.example.andrd_ado_vdo_tkbk_demo.activity.ConfigActivityHandler;
...
public class MyMediaPocsThrd extends MediaPocsThrd
{
    ...
    public class Ntwk //存放网络。
    {
        public String m_IPAddrStrPt; //存放IP地址字符串的指针。
        public String m_PortStrPt; //存放端口字符串的指针。
        ...
        public int m_UseWhatXfrPrtcl; //存放使用什么传输协议，为0表示TCP协议，为1表示UDP协议。
        public int m_IsCreateSrvrOrClnt; //存放创建服务端或者客户端标记，为1表示创建服务端，为0表示创建客户端。
        public TcpSrvrSokt m_TcpSrvrSoktPt; //存放本端TCP协议服务端套接字的指针。
        public TcpClntSokt m_TcpClntSoktPt; //存放本端TCP协议客户端套接字的指针。
        public AudpSokt m_AudpSoktPt; //存放本端高级UDP协议套接字的指针。
        public HTLong m_AudpCnctIdxPt; //存放本端高级UDP协议连接索引。
    }
    public Ntwk m_NtwkPt = new Ntwk();
    enum PktTyp //数据包类型。
    {
        AllowCnct, //允许连接包。
        RefuseCnct, //拒绝连接包。
    ...
    public int m_IsAutoAllowCnct; //存放是否自动允许连接，为0表示手动，为1表示自动。
    public int m_RqstCnctRslt; //存放请求连接的结果，为0表示没有选择，为1表示允许，为2表示拒绝。
    ...
    public MyMediaPocsThrd(ConfigVoIPActivity MainActivityPt, Handler MainActivityHandlerPt )
    {
    ...
    @Override public int UserInit()
    {
        ...
        Out:
        {
            ...
            while( true )
            {
                if( m_NtwkPt.m_IsCreateSrvrOrClnt == 1 ) //如果是服务端。
                {
                    if( m_RqstCnctRslt == 1 ) //如果允许连接。
                    {
                        ...
                        {
                            {Message p_MessagePt = new Message();p_MessagePt.what = ConfigActivityHandler.Msg.RqstCnctDlgDstoy.ordinal();m_MainActivityHandlerPt.sendMessage( p_MessagePt );} //向主界面发送毁请求连接对话框的消息。
                ...
                {
                    {Message p_MessagePt = new Message();p_MessagePt.what = ConfigActivityHandler.Msg.RqstCnctDlgDstoy.ordinal();m_MainActivityHandlerPt.sendMessage( p_MessagePt );} //向主界面发送销毁请求连接对话框的消息。
            ...
            if( m_NtwkPt.m_XfrMode == 0 ) {Message p_MessagePt = new Message();p_MessagePt.what = ConfigActivityHandler.Msg.PttBtnInit.ordinal();m_MainActivityHandlerPt.sendMessage( p_MessagePt );} //向主界面发送初始化一键即按即通按钮的消息。
    ...
    @Override public void UserDstoy()
    {
        ...
        if( m_NtwkPt.m_IsCreateSrvrOrClnt == 1 ) //如果是创建服务端。
        {
            if( m_IsRecvExitPkt == 1 )
            {
                ...
                RqirExit( 2, 0 ); //请求重启。
                {Message p_MessagePt = new Message();p_MessagePt.what = ConfigActivityHandler.Msg.Vibrate.ordinal();m_MainActivityHandlerPt.sendMessage( p_MessagePt );} //向主界面发送振动的消息。
                if( m_NtwkPt.m_XfrMode == 0 ) {Message p_MessagePt = new Message();p_MessagePt.what = ConfigActivityHandler.Msg.PttBtnDstoy.ordinal();m_MainActivityHandlerPt.sendMessage( p_MessagePt );} //向主界面发送销毁一键即按即通按钮的消息。
            }
